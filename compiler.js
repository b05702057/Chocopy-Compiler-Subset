...
function codeGenField(expr, globalEnv, localEnv) {
    ...
    return __spreadArray(__spreadArray([], objAddr, true), ["(i32.const ".concat(indexOfField * 4, ") \n(i32.add)"), "(i32.load)"], false);
}
function codeGenCall(expr, globalEnv, localEnv) {
    ...
    if (globalEnv.classInits.has(expr.name)) {
        ...
        return __spreadArray(__spreadArray([], initVals_1, true), [
            "(global.get $heap)",
            "(global.get $heap)",
            "(i32.const ".concat(classIndexes.size * 4, ")"),
            "(i32.add)",
            "(global.set $heap)",
            "(call $$".concat(expr.name, "$__init__)"),
        ], false);
    }
    var codes = [];
    // collect arguments
